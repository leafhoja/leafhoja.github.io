...
function initVocabTab() {
  ...
  groups.forEach(function (g) {
    ...
    pills.forEach(function (p) {
      ...
      pill.title = p.def;
      pill.innerHTML =
        '<span class="s2-vp-es">' + esc(p.es) + '</span>' +
        '<span class="s2-vp-def">' + esc(p.shortDef) + '</span>';
...
function openWordModal(esText, defText, posOverride) {
  ...
  var body = '<div class="dict-ja-big">' + esc(defText) + '</div>';

  if (entry.note) {
    body += '<div class="dict-note">📝 ' + esc(entry.note) + '</div>';
  }
  if (entry.vtype) {
    body += '<div class="dict-row"><span class="dict-label">動詞型</span><span class="dict-val">' + esc(entry.vtype) + '</span></div>';
  }
  if (entry.conj) {
  ...
  if (entry.example) {
    body += '<div class="dict-example">' +
      ...
      '<div class="dict-example-es">' + esc(entry.example.es) + '</div>' +
      '<div class="dict-example-ja">' + esc(entry.example.ja) + '</div>' +
      '</div>';
  }
...
// esc() は spanish1-search.js で定義（一列・二列共通）
